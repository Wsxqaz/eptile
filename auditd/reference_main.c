...
void original_audit_log_end(struct audit_buffer *ab);
void hook_audit_log_end(struct audit_buffer *ab);
...
// Define the correct type for the hook function
typedef void (*audit_log_end_hook_t)(struct audit_buffer *ab);

// Simplified approach - instead of finding the exact hook, we'll directly modify the function
void *original_hook_addr = NULL;

// Modified hook function for audit_log_end
void hook_audit_log_end(struct audit_buffer *ab) {
    printk(KERN_INFO "hook_audit_log_end\n");
    
    if (ab) {
        printk(KERN_INFO "audit_buffer: %px\n", ab);
        printk(KERN_INFO "Processing audit log\n");
        
        // Add your audit log processing logic here
        // For example, you could examine the audit buffer contents
    }
    
    // Call the original function (which was saved)
    ((void (*)(struct audit_buffer *))((char *)original_audit_log_end + ORIG_LEN))(ab);
}
...
// Override the function instead of directly hooking it
long run_hook(void * addr, int len) {
    ORIG_LEN = len;
    original_hook_addr = addr;

    // Save the original function implementation
    memcpy(original_audit_log_end, addr, len);
    x86_put_jmp(original_audit_log_end + len, original_audit_log_end + len, addr + len);
    
    // Debug prints
    for (int i = 0; i < 20; i++) {
        printk(KERN_INFO "pre orig [%d]: 0x%02x ", i, ((unsigned char *)original_audit_log_end)[i]);
    }
    
    // Use direct function hook
    printk(KERN_INFO "Using direct function hook\n");
    x86_put_jmp(addr, addr, hook_audit_log_end);
...
{
    void * addr = (void *)find_kallsym("audit_log_end");
    printk(KERN_INFO "audit_log_end: %px\n", addr);

    if (addr == 0) {
        printk(KERN_INFO "audit_log_end not found\n");
        return 0;
    }

    int len = lde_get_length((void *)addr);
    printk(KERN_INFO "audit_log_end len: %d\n", len);
    while (len < 5) {
...
long hook_reset(void *data, int len) {
    printk(KERN_INFO "Restoring original audit_log_end hook\n");

    // Restore the original function bytes
    for (int i = 0; i < len; i++) {
        ((unsigned char *)data)[i] = ((unsigned char *)original_audit_log_end)[i];
    }
